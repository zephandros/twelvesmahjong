...
import {
  MENU_TRACK, SELECT_TRACK, GAME_TRACKS, musicUrl, MUSIC_TITLES,
...
describe('assets de audio (public/{music,sfx,voices})', () => {
  it('existe cada tema de música (menú + selección + partida)', () => {
    for (const track of [MENU_TRACK, SELECT_TRACK, ...GAME_TRACKS]) {
      expect(has(musicUrl(track)), `falta ${musicUrl(track)}`).toBe(true)
    }
  })

  it('MUSIC_TITLES cubre exactamente los temas del catálogo', () => {
    const tracks = [MENU_TRACK, SELECT_TRACK, ...GAME_TRACKS].sort()
